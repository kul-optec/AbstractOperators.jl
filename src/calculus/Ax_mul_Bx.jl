...
function mul!(y, J::AdjointOperator{Ax_mul_BxJac{L1,L2,C,D}}, b) where {L1,L2,C,D}
  ...
  mul!(J.A.bufD, J.A.A', J.A.bufC)
  y .+= J.A.bufD
  return y
end

size(P::Ax_mul_Bx) = ((size(P.A,1)[1],size(P.A,1)[1]),size(P.A,2))
size(P::Ax_mul_BxJac) = ((size(P.A,1)[1],size(P.A,1)[1]),size(P.A,2))

fun_name(L::Ax_mul_Bx) = fun_name(L.A)*"*"*fun_name(L.B) 
fun_name(L::Ax_mul_BxJac) = fun_name(L.A)*"*"*fun_name(L.B) 

domainType(L::Ax_mul_Bx)   = domainType(L.A)
codomainType(L::Ax_mul_Bx) = codomainType(L.A)

domainType(L::Ax_mul_BxJac)   = domainType(L.A)
codomainType(L::Ax_mul_BxJac) = codomainType(L.A)
