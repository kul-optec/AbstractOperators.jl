...
immutable Jacobian{T <: NonLinearOperator, X<:AbstractArray} <: LinearOperator
	A::T
	x::X
end

#Jacobian of LinearOperator 
Jacobian{T<:LinearOperator,X<:AbstractArray}(L::T,x::X) = L
#Jacobian of Scale
...
domainType(L::Jacobian) = codomainType(L.A)
codomainType(L::Jacobian) = domainType(L.A)
