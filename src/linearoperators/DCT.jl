...
```
"""
struct DCT{N,C<:RealOrComplex,T1<:AbstractFFTs.Plan,T2<:AbstractFFTs.Plan} <:
	   CosineTransform{N,C,T1,T2}
	dim_in::NTuple{N,Int}
	A::T1
	At::T2
end
...
```
"""
struct IDCT{N,C<:RealOrComplex,T1<:AbstractFFTs.Plan,T2<:AbstractFFTs.Plan} <:
	   CosineTransform{N,C,T1,T2}
	dim_in::NTuple{N,Int}
	A::T1
	At::T2
